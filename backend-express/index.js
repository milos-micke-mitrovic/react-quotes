...
const QUOTES = [
  ...
  },
  {
    ...
    upvotedBy: ['d964099a-1a2a-46f5-9782-e2601b5aac9e', 'a77d280e-94b8-4a1e-a869-1f14e622fa4e'],
    downvotedBy: ['3de3d9ff-60b1-4694-9e87-77aefea9ea0e'],
  },
  {
...
app.get('/quotes', (req, res) => {
  ...
  res.send({
    ...
    quotes: quotes.slice(start, end).map(quote => transformQuote(quote, user.id)),
  })
})
...
app.post('/quotes', (req, res) => {
  ...
  if (content && typeof content != 'string') errors.push({ key: 'content', value: 'wrongType' })
  if (!author) errors.push({ key: 'author', value: 'required' })
  if (author && typeof author != 'string') errors.push({ key: 'author', value: 'wrongType' })
  if (!tags) errors.push({ key: 'tags', value: 'required' })
  if (tags && !Array.isArray(tags)) errors.push({ key: 'tags', value: 'wrongType' })

  if (errors.length > 0) {
    const error = errors.reduce((acc, curr) => ({ ...acc, [curr.key]: curr.value }), {})
...
app.post('/quotes/:id/upvote', (req, res) => {
  ...
  if (quote.upvotedBy.includes(user.id)) return res.status(400).send({ error: `Already upvoted.` })
  if (quote.downvotedBy.includes(user.id)) return res.status(400).send({ error: `Delete the existing downvote in order to upvote.` })
...
app.delete('/quotes/:id/upvote', (req, res) => {
  ...
  if (!quote.upvotedBy.includes(user.id)) return res.status(400).send({ error: `No upvote to delete.` })
...
app.post('/quotes/:id/downvote', (req, res) => {
  ...
  if (quote.upvotedBy.includes(user.id)) return res.status(400).send({ error: `Delete the existing upvote in order to downvote.` })
  if (quote.downvotedBy.includes(user.id)) return res.status(400).send({ error: `Already downvoted.` })
...
app.delete('/quotes/:id/downvote', (req, res) => {
  ...
  if (!quote.downvotedBy.includes(user.id)) return res.status(400).send({ error: `No downvote to delete.` })
